...
A comprehensive, high-performance assembly function library and structured control-flow macro package for the **Intel 8080, Intel 8085, and Zilog Z80** microprocessors running Digital Research's **CP/M 2.2** operating system ([Operating System Reference](REFERENCES-v2.md#1-operating-system--system-architecture-specifications)).

Written in standard 8080 assembly instructions to ensure universal portability across all 8080/8085/Z80 retro-hardware and modern emulators, this package provides a complete software development kit for low-level systems programming, embedded retro-computing, and application development ([Architecture Specifications](REFERENCES-v2.md#1-operating-system--system-architecture-specifications)).
...
High-level control-flow constructs for assembly language without performance overhead ([Language Reference](REFERENCES-v2.md#2-assembly-subroutine-libraries--textbooks)):
...
* **IOBYTE Device Redirection**: `GET_IOBYTE`, `SET_IOBYTE`, `PR_IOBYTE`, and field setters (`SET_CON_DEV`, `SET_RDR_DEV`, `SET_PUN_DEV`, `SET_LST_DEV`) to dynamically manipulate page-zero IOBYTE (`0003H`) mappings for logical devices (`CON:`, `RDR:`, `PUN:`, `LST:`) ([IOBYTE Specification](REFERENCES-v2.md#1-operating-system--system-architecture-specifications)).

### 3. Extended Mathematics & Numeric Packages
* **32-Bit & 64-Bit Binary Integer Math**: Multi-byte addition, subtraction, shift-and-add multiplication, restoring division, sign extension, and formatted unsigned decimal string printing (`ADD32`, `MUL32`, `DIV32`, `PRDEC32`, `ADD64`, `MUL64`, `DIV64`, `PRDEC64`) ([Subroutine Textbook References](REFERENCES-v2.md#2-assembly-subroutine-libraries--textbooks)).
* **16.16 Signed Fixed-Point Arithmetic**: 32-bit fixed-point binary arithmetic (`FIX_ADD`, `FIX_SUB`, `FIX_MUL`, `FIX_DIV`), integer conversion, and 4-digit fractional decimal output (`PR_FIX`) ([Fixed-Point Algorithms](REFERENCES-v2.md#2-assembly-subroutine-libraries--textbooks)).
* **Packed BCD Math Package**: Multi-byte packed Binary-Coded Decimal addition (`BCD_ADD`), 10's complement subtraction (`BCD_SUB`), `DAA` hardware acceleration, integer-to-BCD conversion (`INT_TO_BCD`), and leading-zero suppressed decimal printing (`PR_BCD`) ([BCD Math References](REFERENCES-v2.md#2-assembly-subroutine-libraries--textbooks)).
* **32-Bit Software Floating-Point Math**: 5-byte floating accumulator (`FAC1`, `FAC2`) supporting normalized arithmetic (`FP_ADD`, `FP_SUB`, `FP_MUL`, `FP_DIV`), integer conversion, and formatted ASCII output (`PR_FLOAT`) ([Floating-Point Algorithms](REFERENCES-v2.md#2-assembly-subroutine-libraries--textbooks)).

### 4. Memory, String & Integrity Tools
* **Dynamic Heap Allocator**: First-fit memory manager (`MALLOC`, `FREE`, `REALLOC`, `CALLOC`) featuring 4-byte headers, TPA/BDOS memory boundary auto-detection, block splitting, contiguous free-list coalescing (`MALLOC_COALESCE`), and heap statistics reporting (`PR_HEAP_INFO`) ([Heap Allocation Reference](REFERENCES-v2.md#1-operating-system--system-architecture-specifications)).
* **Extended String Utilities**: Substring search (`STRSTR`), wildcard pattern matching (`STRMATCH` with `?` and `*`), in-place tokenization (`STRTOK`), whitespace trimming (`STRTRIM`), and case conversion (`STRUPR`, `STRLWR`) ([String Subroutine Reference](REFERENCES-v2.md#2-assembly-subroutine-libraries--textbooks)).
* **CRC-16-CCITT Data Integrity**: Bit-stream CRC-16 checksum calculation (`0x1021` polynomial) for memory buffers (`CRC16_BUF`) and raw disk files (`CRC16_FILE`) ([CRC-16 Algorithms](REFERENCES-v2.md#2-assembly-subroutine-libraries--textbooks)).

### 5. Hardware Interfacing & Terminal Control
* **Intel 8251 USART Driver**: Hardware serial port setup (`UART_INIT`), dynamic runtime port remapping via self-modifying code (`UART_SET_PORTS`), polled I/O, and a 64-byte software RX ring buffer (`UART_BUF_PUT`, `UART_BUF_GET`) ([Intel 8251 Manual](REFERENCES-v2.md#4-hardware-peripheral--auxiliary-component-manuals)).
* **Command-Line Argument Parser**: Parses CP/M command tail bytes at `0080H` into structured `ARGC`/`ARGV` arrays (`ARG_INIT`, `ARG_GET`, `ARG_FIND_FLAG`), supporting quoted strings and command-line flags ([CP/M Command Tail Reference](REFERENCES-v2.md#1-operating-system--system-architecture-specifications)).
...
This library is compatible with standard CP/M 8080/Z80 macro assemblers and development toolchains ([Toolchain References](REFERENCES-v2.md#3-toolchains-linkers-and-emulator-references)):
